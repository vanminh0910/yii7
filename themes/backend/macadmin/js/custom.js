...
$(document).ready(function() {
    $('.change-pageSize').live('change', function() {
        $.fn.yiiGridView.update('data-table-paging',
                {data: {pageSize: $(this).val()},
                    url: $(this).attr('url')})
    });
    //open save menu
    var idMenu = getCookie('id_menu');
    if (idMenu !== null) {
        $("#" + idMenu).addClass('open');
        $("#" + idMenu + " a span i").attr('class', 'fa fa-chevron-down');
...
//        var id = menu_li.attr('id');

        if (!$(this).attr('id')) {
            $("#nav > li > ul").slideUp(350);
            $("#nav > li").removeClass("open");
//            $("#" + id + " a span i").attr('class', 'fa fa-chevron-right');
            setCookie('id_menu', 'null', 10);
        }
        else {
            if(getCookie('id_menu')!= $(this).attr('id')){
                $("#nav > li > ul").slideUp(350);
                $("#nav > li").removeClass("open");
                $(this).children("ul").slideDown(350);
                $(this).addClass("open");
                $("#" + $(this).attr('id') + " a span i").attr('class', 'fa fa-chevron-down');
                setCookie('id_menu', $(this).attr('id'), 10);
            }
