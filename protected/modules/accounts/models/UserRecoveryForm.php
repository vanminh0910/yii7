...
class UserRecoveryForm extends CFormModel {
    ...
    public function attributeLabels() {
        return array(
            'login_or_email' => AccountsModule::t("Username or email"),
        );
    }
    ...
    public function checkexists($attribute, $params) {
        if (!$this->hasErrors()) {  // we only want to authenticate when no input errors
            ...
            if ($user === null) {
                if (strpos($this->login_or_email, "@")) {
                    $this->addError("login_or_email", AccountsModule::t("Email is incorrect."));
                } else {
                    $this->addError("login_or_email", AccountsModule::t("Username is incorrect."));
                }
            }
        }
    }
