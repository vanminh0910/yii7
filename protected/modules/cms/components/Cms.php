...
class Cms extends CApplicationComponent {
    ...
    public function pageHeader($name = 'index') {
        ...
        if (isset($page->content->metaTitle)) {
            ...
            foreach ($page->contents as $v => $k) {
                $cs->registerLinkTag('alternate', null, Yii::app()->createAbsoluteUrl('//site/' . $name, array('lang' => $k->locale))
    ...
    public function isActive($name) {
        ...
        return ($controller->module !== null && $controller->module->id === 'cms' && $controller->id === 'page' && $controller->action->id === 'view' && isset($_GET['id']) && $_GET['id'] === $page->id) || $this->isChildActive($page);
    }
