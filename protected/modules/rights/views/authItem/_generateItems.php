    ...
    <?php foreach ($items['controllers'] as $key => $item): ?>
        ...
        <?php if (isset($item['actions']) === true && $item['actions'] !== array()): ?>
            ...
            <tr class="controller-row <?php echo $controllerExists === true ? 'exists' : ''; ?>">
                <td class="checkbox-column"><?php echo $controllerExists === false ? $form->checkBox($model, 'items[' . $controllerKey . '.*]', array('style' => 'margin-right:5px')) : ''; ?></td>
            ...
            foreach ($item['actions'] as $action):
                ...
                <tr class="action-row<?php echo $actionExists === true ? ' exists' : ''; ?><?php echo ($i++ % 2) === 0 ? ' odd' : ' even'; ?>">
                    <td class="checkbox-column"><?php echo $actionExists === false ? $form->checkBox($model, 'items[' . $actionKey . ']', array('style' => 'margin-right:5px')) : ''; ?></td>
