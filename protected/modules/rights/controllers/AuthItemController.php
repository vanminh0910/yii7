...
class AuthItemController extends RController {
    ...
    public function actionPermissions() {
        ...
        $authItem->setTypes(array(CAuthItem::TYPE_OPERATION, CAuthItem::TYPE_TASK));

        if (isset($_GET['AuthItem'])) {
            $authItem->name = $_GET['AuthItem']['name'];
        }
        if (isset($_GET['pageSize'])) {
            $authItem->pageSize = $_GET['pageSize'];
        }

        // Render the view
        $this->render('permissions', array('authItem' => $authItem,
        ));
...
//        isset($_POST['ajax']) === true ? $this->renderPartial($view, $params) : $this->render($view, $params);
    ...
    public function actionOperations() {
        $authItem = new AuthItem();
        $authItem->setTypes(CAuthItem::TYPE_OPERATION);

        if (isset($_GET['AuthItem'])) {
            $authItem->name = $_GET['AuthItem']['name'];
            $authItem->description = $_GET['AuthItem']['description'];
            $authItem->bizrule = $_GET['AuthItem']['bizrule'];
        }
        if (isset($_GET['pageSize'])) {
            $authItem->pageSize = $_GET['pageSize'];
        }
        ...
        // Render the view
        $this->render('operations', array('authItem' => $authItem
        ));
    }
    ...
    public function actionTasks() {
        $authItem = new AuthItem();
        $authItem->setTypes(CAuthItem::TYPE_TASK);
        ...
        // Render the view
        $this->render('tasks', array('authItem' => $authItem
        ));
    }
    ...
    public function actionRoles() {
        $authItem = new AuthItem();
        $authItem->setTypes(CAuthItem::TYPE_ROLE);
        ...
        // Render the view
        $this->render('roles', array('authItem' => $authItem
        ));
    }
    ...
    public function actionCreate() {
        ...
        if (isset($_POST['AuthItemForm']) === true) {
            ...
            if ($formModel->validate() === true) {
                ...
                $this->redirect($this->createAbsoluteUrl(Rights::getAuthItemRoute($type)[0]));
            }
        }
    ...
    public function actionUpdate() {
        ...
        if (isset($_POST['AuthItemForm']) === true) {
            ...
            if ($formModel->validate() === true) {
                ...
                // Redirect to the correct destination
                $this->redirect($this->createAbsoluteUrl(Rights::getAuthItemRoute($model->type)[0]));
            }
        }
...
//        );
        $urlReturn = $_GET['urlReturn'];


        $this->redirect(Yii::app()->user->getRightsReturnUrl(array('authItem/' . $urlReturn)));
    }
    ...
    public function createDeleteButton($data) {
        $type = $data->type;
        $message = '';
        $typeString = '';
        if ($type === CAuthItem::TYPE_ROLE . '') {
            $typeString = 'roles';
            $message = Rights::t('core', 'Are you sure you want to delete this role?');
        } else if ($type === CAuthItem::TYPE_TASK . '') {
            $typeString = 'tasks';
            $message = Rights::t('core', 'Are you sure you want to delete this task?');
        } else {
            $typeString = 'operations';
            $message = Rights::t('core', 'Are you sure you want to delete this operation?');
        }
        $url = $this->createAbsoluteUrl('delete', array('name' => urlencode($data->name), 'urlReturn' => $typeString));
        $result = $this->renderPartial('button/deleteButton', array('url' => $url, 'message' => $message), true);
        return $result;
    }

    public function generateLinkRole($data, $roleName) {
        $count = AuthItemChild::model()->countByAttributes(array('parent' => $roleName, 'child' => $data->name));
        $type = '';
        if ($count > 0) {
            $type = 'Revoke';
        } else {
            $count = AuthItemChild::model()->isRoleInherited($roleName, $data->name);
            if ($count > 0) {
                return 'Inherited';
            } else {
                $type = 'Assign';
        ...
        $result = $this->renderPartial('button/linkButton', array('type' => $type, 'role' => $roleName, 'child' => $data->name), true);
        return $result;
    }

    public function actionAssignRevoke() {
        // We only allow deletion via POST request

        if (Yii::app()->request->isPostRequest === true) {
            $itemName = $this->getItemName();
            $childName = $this->getChildName();
            $authI = AuthItemChild::model()->findByAttributes(array('parent' => $itemName, 'child' => $childName));
            if (isset($authI)) {
                $authI->delete();
            } else {
                $authI = new AuthItemChild();
                $authI->parent = $itemName;
                $authI->child = $childName;
                $authI->save();
            }
