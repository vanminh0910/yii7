...
class Shop {
    ...
    public static function mailNotification($order) {
        ...
        if ($email !== null) {
            ...
            mail($email, Shop::t('Order #{order_id} has been made in your Webshop', array(
                        '{order_id}' => $order->id)), CHtml::link(Shop::t('direct link'), array(
...
//        echo '</div>';
    ...
    public static function getTotalPriceNumber() {
        $price_total = 0;
        $tax_total = 0;
        foreach (Shop::getCartContent() as $product) {
            $model = Products::model()->findByPk($product['product_id']);
